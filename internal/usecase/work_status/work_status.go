...
func (uc *UseCase) Create(ctx context.Context, workStatus Create, authHeader string) (int64, error) {
	token, err := uc.auth.IsValidToken(ctx, authHeader)
	if err != nil {
		return 0, err
	}

	return uc.repo.Create(ctx, workStatus, token.Id)
}

func (uc *UseCase) Delete(ctx context.Context, workStatusId int64, authHeader string) error {
	token, err := uc.auth.IsValidToken(ctx, authHeader)
	if err != nil {
		return err
	}

	return uc.repo.Delete(ctx, workStatusId, token.Id)
}

func (uc *UseCase) GetById(ctx context.Context, id int64) (WorkStatusById, error) {
