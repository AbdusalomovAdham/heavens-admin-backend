...
func (uc *UseCase) Create(ctx context.Context, department Create, authHeader string) (int64, error) {
	token, err := uc.auth.IsValidToken(ctx, authHeader)
	if err != nil {
		return 0, err
	}

	return uc.repo.Create(ctx, department, token.Id)
}

func (uc *UseCase) Delete(ctx context.Context, departmentId int64, authHeader string) error {
	token, err := uc.auth.IsValidToken(ctx, authHeader)
	if err != nil {
		return err
	}

	return uc.repo.Delete(ctx, departmentId, token.Id)
}

func (uc *UseCase) GetById(ctx context.Context, id int64) (DepartmentById, error) {
